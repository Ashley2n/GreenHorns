...
class User(Base, UserMixin, db.Model):
    ...
    xp = Column(Float, nullable=False)
    created_at = Column(DateTime, default=datetime.now)
    image_path = Column(String(100), nullable=False)

class Campaign(Base, db.Model):
...
class Cached_Recipes(Base, db.Model):
...
class Game_Sessions(Base, db.Model):
